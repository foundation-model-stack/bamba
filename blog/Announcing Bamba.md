...
We introduce Bamba9B, an inference efficient Hybrid Mamba2 model trained by IBM, Princeton, CMU, and UIUC on completely open data. The model demonstrates 2.5x throughput improvement and 2x latency compared to standard transformers in vLLM. The model is enabled in transformers, vLLM, TRL, and llama.cpp.
...
Transformer models are increasingly used in real-world applications but face GPU memory bandwidth bottlenecks during inference, particularly in per-token decoding, which is worsened with longer context length models that increase the KV-cache state linearly. Techniques like lower precision, layer pruning, and compression can alleviate the problem, but do not address the root cause.

Emerging architectures such as Mamba, Griffin, and DeltaNet eliminate this bottleneck by making KV-cache constant. Mamba architecture has gained significant traction in the recent past. For example, Jamba and Samba interleave Mamba layers with transformer layers. Pure Mamba2 models like Codestral Mamba demonstrate state-of-the-art (SOTA) results on coding tasks, while NVIDIA’s Mamba2 hybrid achieves competitive performance across long-context and traditional LLM benchmarks. Recent innovations, like Falcon Mamba, achieve SOTA rankings on Hugging Face leaderboards.
...
We divide our evaluations into three parts: first, we compare with SoTA transformer models, second we compare with transformer models with similar token budget, and finally, we compare with other Mamba variants.
